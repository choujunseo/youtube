...
import { hallOfFameGateAdGroupId } from '@/lib/adGroupIds';
import { recordAdImpression } from '@/services/adLogService';
import { useAuthStore } from '@/store/authStore';
import { fireTossHaptic, isIntegratedAdSupported, showIntegratedFullScreenGateAd } from '@/utils/tossBridge';

const PULL_MAX = 92;
const PULL_TRIGGER = 62;
const HALL_GATE_COOLDOWN_MS = 1 * 60 * 60 * 1000;
...
export default function HallOfFamePage() {
  ...
  const hallGateAdGroupId = hallOfFameGateAdGroupId();
    ...
    void showIntegratedFullScreenGateAd(hallGateAdGroupId).then((shown) => {
      ...
      if (userId) {
        void recordAdImpression({
          userId,
          adType: 'hall_gate_interstitial',
          adGroupId: hallGateAdGroupId,
          rewardAmount: 0,
        }).catch((err: unknown) => {
          console.error('[HallOfFamePage] record_ad_impression failed', err);
        });
      }
      try {
