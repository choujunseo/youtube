...
function ideaIdFromPayload(payload: Record<string, unknown>): string | null {
  const raw = payload.idea_id;
...
function NotificationCard(props: INotificationCardProps) {
  ...
  const ideaId = ideaIdFromPayload(item.payload);
    ...
    <>
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          {ideaId ? (
            <p className="mt-1 text-xs font-medium text-blue-600">아이디어 보기</p>
          ) : null}
        </div>
      </div>
    </>
